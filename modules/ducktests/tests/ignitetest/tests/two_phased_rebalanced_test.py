...
from ignitetest.services.utils.util import copy_file_to_logs
...
class TwoPhasedRebalancedTest(IgniteTest):
    ...
    def two_phased_rebalancing_test(self, ignite_version):
        ...
        dump_1 = fix_data(control_utility, node)
        ...
        dump_2 = fix_data(control_utility, node)
    ...
    def start_cells(self, ignite_version: str, cells_cnt: int, cell_nodes_cnt: int, cache_name: str,
        ...
        cache_cfg = CacheConfiguration(name=cache_name, backups=self.NUM_CELL-1, affinity=Affinity())
...
def fix_data(control_utility: ControlUtility, node):
    """
    ...
    return copy_file_to_logs(node, dump, control_utility._cluster.log_dir)
